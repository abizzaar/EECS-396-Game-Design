...
public class ProjectileThrower : MonoBehaviour {
    ...
    {
        Rigidbody2D[] allobjects =  GameObject.FindObjectsOfType<Rigidbody2D>();
        foreach (Rigidbody2D a in allobjects){
            if (IsActive(a))
            {
                return true;
            }
    ...
    {
        FireControl();
        if ((firingState != FiringState.Aiming && firingState != FiringState.Idle && !WaitingForPhysicsToSettle()) || Input.GetKeyDown(KeyCode.Escape)){
            ResetForFiring();
        }
