...
namespace Assets.Code
{
    ...
    public class AsteroidManager : MonoBehaviour, ISaveLoad
    {
        ...
        {
            GameObject asteroid = Object.Instantiate((GameObject) _asteroidPrefab, pos, rotation, _holder);
            asteroid.GetComponent<Asteroid>().Initialize(velocity, size);
        }
        ...
        public void OnLoad (GameData data) {
            foreach (Transform a in _holder) 
            {
                Destroy(a.gameObject);
            }
            AsteroidsData allData = (AsteroidsData) data;
            foreach (AsteroidData a in allData.Asteroids)
            {
                ForceSpawn(a.Pos, a.Velocity, a.Size);
            }
        }
